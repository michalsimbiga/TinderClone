...
internal fun MyApiException.toMyError(): MyError = when {
    this.code == HttpURLConnection.HTTP_UNAUTHORIZED -> MyError.Unauthorized(
        this
    )
    this.code == HttpURLConnection.HTTP_NO_CONTENT -> MyError.NoBody(
        this
    )
    this.code == HttpURLConnection.HTTP_BAD_REQUEST -> badRequestToMyError(
        this
    )
    this.code >= HttpURLConnection.HTTP_INTERNAL_ERROR -> MyError.ServerError(
        this
    )
